...
import com.fh.utils.CookieUtil;
...
public class ImageCodeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//如果sessionId为空的话就给重新写入
		if(StringUtils.isEmpty(fhId)){
			//生成uuid
			fhId= UUID.randomUUID().toString().replace("-", "").toUpperCase();
			//写道cookie中
			CookieUtil.writeCookie("fhId",fhId,"admin.shop.com",-1,response);
		}
		//开辟空间加入redis
		RedisUtil.set("code:"+fhId,scode);
		RedisUtil.expire("code:"+fhId,1*60);
